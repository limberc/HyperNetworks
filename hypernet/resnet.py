...
class HyperModule(nn.Module):
    ...
    def forward(self, z):
        h_in = self.proj1(z)
        h_in = F.relu(h_in, True)
        h_in = h_in.view(self.in_size, self.z_dim)

        h_final = self.proj2(h_in)
        h_final = F.relu(h_final, True)
    ...
    def __init__(self,
        ...
        for i in range(self.out_channels // self.base):
            for j in range(max(1, self.in_channels // self.base)):
                self.layer_embed.append(nn.Parameter(
                    torch.fmod(torch.randn(self.z_dim), 2)))
...
class HyperBasicBlock(BasicBlock):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        super().__init__(inplanes, planes, stride, downsample, groups, base_width,
                         dilation, norm_layer)
        self.hope = HyperModule(z_dim=z_dim, out_size=base, in_size=base)
        self.conv1 = HyperConv2d(inplanes, planes, 3, stride, 1, hyper_module=self.hope, base=base, z_dim=z_dim)
        self.conv2 = HyperConv2d(planes, planes, 3, 1, 1, hyper_module=self.hope, base=base, z_dim=z_dim)


class HyperBottleneck(Bottleneck):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, z_dim=512, base=16):
        super(HyperBottleneck, self).__init__(inplanes, planes, stride, downsample, groups, base_width,
                                              dilation, norm_layer)
        self.hope = HyperModule(z_dim=z_dim, out_size=base, in_size=base)
        width = int(planes * (base_width / 64.)) * groups
        self.conv1 = HyperConv2d(inplanes, width, 1, 1, hyper_module=self.hope, base=base, z_dim=z_dim)
        self.conv2 = HyperConv2d(width, width, 3, stride, dilation, dilation, groups, bias=False,
                                 hyper_module=self.hope, base=base, z_dim=z_dim)
        self.conv3 = HyperConv2d(width, planes * self.expansion, 1, 1, hyper_module=self.hope, base=base, z_dim=z_dim)
...
def resnet18(num_classes=100, base=64, z_dim=512):
    return HyperResNet(HyperBasicBlock, [2, 2, 2, 2], num_classes, base=base, z_dim=z_dim)


def resnet34(num_classes=100, base=64, z_dim=512):
    return HyperResNet(HyperBasicBlock, [3, 4, 6, 3], num_classes, base=base, z_dim=z_dim)
